  ...
  <v-app id="inspire">
    <v-app-bar app color="#0E335F" text dark flat>
      <v-container class="py-0 fill-height">
        ...
        <v-btn text>|</v-btn>
        <v-btn text :to="{name: 'Livro'}">Livros</v-btn>
        ...
        <v-row>
          ...
          <v-col cols="9">
                    ...
                    <v-dialog v-model="dialog" persistent max-width="600px">
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn color="error" block dark v-bind="attrs" v-on="on">
                          <v-icon class="mr-1">mdi-plus-circle</v-icon>Autor
                        </v-btn>
                      </template>
                      <v-card>
                        <v-toolbar color="#0E335F" dark>Adicionar Autor</v-toolbar>
                        <!-- <v-card-title>
            ...
            <v-sheet min-height="70vh" rounded="lg">
              <h2 class="pa-2">Lista de Autores</h2>
              <v-simple-table>
...
export default {
  ...
  created() {
    axios.get('http://127.0.0.1:8000/api/autores').then((data) => {
